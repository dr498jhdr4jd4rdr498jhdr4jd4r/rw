...
async def explore(q: str = "brazzers", page: int = 1):
    ...
    for block in blocks[1:]:
        ...
        if vkey_match and title_match and thumb_match:
            ...
            dur = "HD"
            if dur_match:
                dur = dur_match.group(1) if dur_match.group(1) else dur_match.group(2)
            ...
            if not is_ad and not any(v['vkey'] == vkey for v in videos):
                videos.append({
                    ...
                    "title": title,
                    "thumbnail": thumb,
                    "duration": dur.strip() if dur else "HD",
...
async def extract(url: str):
    ...
    if not vkey_match:
        return JSONResponse({"status": "error", "error": "Invalid viewkey in URL"}, status_code=400)
    ...
    try:
        ...
        return JSONResponse({"status": "error", "error": f"Upstream error: {str(e)}"}, status_code=502)
...
async def proxy_m3u8(url: str, request: Request, request_obj: Request):
    ...
    # We construct the proxy path so it routes correctly when sent back to Cloudflare
